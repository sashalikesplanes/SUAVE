    ...
    if settings.parallel_computing:
        ...
        if np.all(Omega == 0):
            ...
            prod_x = partial(compute_rotor_section_boundary_layers,
                             ...
                             U_blade= U_blade,
                             rho_blade= rho_blade,
                             blade_chords= blade_chords,
                             num_mic= num_mic,
                             num_rot= num_rot,
                             num_cf=  num_cf) 
            boundary_layer  = pool.map(prod_x, data_list)              
            
            for cp_i in range(num_cpt):
    
                delta[cp_i,:,:,:,:,:,0]        = boundary_layer[cp_i].lower_surface_theta     
                delta[cp_i,:,:,:,:,:,1]        = boundary_layer[cp_i].upper_surface_theta     
                delta_star[cp_i,:,:,:,:,:,0]   = boundary_layer[cp_i].lower_surface_delta     
                delta_star[cp_i,:,:,:,:,:,1]   = boundary_layer[cp_i].upper_surface_delta     
                dp_dx[cp_i,:,:,:,:,:,0]        = boundary_layer[cp_i].lower_surface_delta_star
                dp_dx[cp_i,:,:,:,:,:,1]        = boundary_layer[cp_i].upper_surface_delta_star
                Ue[cp_i,:,:,:,:,:,0]           = boundary_layer[cp_i].lower_surface_cf        
                Ue[cp_i,:,:,:,:,:,1]           = boundary_layer[cp_i].upper_surface_cf        
                tau_w[cp_i,:,:,:,:,:,0]        = boundary_layer[cp_i].lower_surface_dp_dx     
                tau_w[cp_i:,:,:,:,:,:,1]       = boundary_layer[cp_i].upper_surface_dp_dx     
                Theta[cp_i,:,:,:,:,:,0]        = boundary_layer[cp_i].lower_surface_Ue        
                Theta[cp_i,:,:,:,:,:,1]        = boundary_layer[cp_i].upper_surface_Ue        
      
      
    
            pool   = multiprocessing.Pool(processes= num_processors)  
            data_list_2 = list(np.arange(num_mic))        
            prod_x_2 = partial(compute_mic_broadband,
                             r = r,
                             num_cpt = num_cpt,
                             num_rot = num_rot,
                             num_azi = num_azi,
                             ...
                             p_ref= p_ref,
                             settings = settings) 
            res  = pool.map(prod_x_2, data_list_2)              
            
            for mi in range(num_mic):
                broadband_noise_results.p_pref_broadband[:,mi,:,:]                               =  res[mi].p_pref_broadband                         
                broadband_noise_results.p_pref_broadband_dBA[:,mi,:,:]                           =  res[mi].p_pref_broadband_dBA                     
                broadband_noise_results.SPL_prop_broadband_spectrum[:,mi,:,:]                    =  res[mi].SPL_prop_broadband_spectrum              
                broadband_noise_results.SPL_prop_broadband_spectrum_dBA[:,mi,:,:]                =  res[mi].SPL_prop_broadband_spectrum_dBA          
                broadband_noise_results.SPL_prop_broadband_1_3_spectrum[:,mi,:,:]                =  res[mi].SPL_prop_broadband_1_3_spectrum          
                broadband_noise_results.SPL_prop_broadband_1_3_spectrum_dBA[:,mi,:,:]            =  res[mi].SPL_prop_broadband_1_3_spectrum_dBA      
                broadband_noise_results.p_pref_azimuthal_broadband[:,mi,:,:,:]                   =  res[mi].p_pref_azimuthal_broadband               
                broadband_noise_results.p_pref_azimuthal_broadband_dBA[:,mi,:,:,:]               =  res[mi].p_pref_azimuthal_broadband_dBA           
                broadband_noise_results.SPL_prop_azimuthal_broadband_spectrum[:,mi,:,:,:]        =  res[mi].SPL_prop_azimuthal_broadband_spectrum    
                broadband_noise_results.SPL_prop_azimuthal_broadband_spectrum_dBA[:,mi,:,:,:]    =  res[mi].SPL_prop_azimuthal_broadband_spectrum_dBA             
...
def compute_mic_broadband(i,r,num_cpt,num_rot,num_azi,num_cf,num_sec,blade_chords,delta_r,M,c_0,Omega,U_inf,rho,bspv,frequency,beta_sq,B,kine_visc,
                          Theta,tau_w,dp_dx,delta,delta_star,Ue,p_ref,settings):
