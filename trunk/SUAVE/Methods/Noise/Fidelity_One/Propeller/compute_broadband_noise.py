...
def compute_broadband_noise(freestream,angle_of_attack,bspv,
    ...
    if np.all(Omega == 0):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers = num_processors) as executor:  
            results = [executor.submit(compute_rotor_section_boundary_layers,i,rotor,num_azi,Re_blade,alpha_blade,num_sec,U_blade,rho_blade,blade_chords,num_mic,num_rot,num_cf)for i in range(num_cpt)] 
            cp_i = 0
            for instance in concurrent.futures.as_completed(results):
                boundary_layer = instance.result()  
 
                delta[cp_i,:,:,:,:,:,0]        = boundary_layer.lower_surface_theta     
                delta[cp_i,:,:,:,:,:,1]        = boundary_layer.upper_surface_theta     
                delta_star[cp_i,:,:,:,:,:,0]   = boundary_layer.lower_surface_delta     
                delta_star[cp_i,:,:,:,:,:,1]   = boundary_layer.upper_surface_delta     
                dp_dx[cp_i,:,:,:,:,:,0]        = boundary_layer.lower_surface_delta_star
                dp_dx[cp_i,:,:,:,:,:,1]        = boundary_layer.upper_surface_delta_star
                Ue[cp_i,:,:,:,:,:,0]           = boundary_layer.lower_surface_cf        
                Ue[cp_i,:,:,:,:,:,1]           = boundary_layer.upper_surface_cf        
                tau_w[cp_i,:,:,:,:,:,0]        = boundary_layer.lower_surface_dp_dx     
                tau_w[cp_i:,:,:,:,:,:,1]       = boundary_layer.upper_surface_dp_dx     
                Theta[cp_i,:,:,:,:,:,0]        = boundary_layer.lower_surface_Ue        
                Theta[cp_i,:,:,:,:,:,1]        = boundary_layer.upper_surface_Ue        
 
                cp_i += 1
            
        with concurrent.futures.ProcessPoolExecutor(max_workers = num_processors) as executor:  
            results = [executor.submit(compute_mic_broadband,i,r,num_cpt,num_rot,num_azi,num_cf,num_sec,blade_chords,delta_r,M,c_0,Omega,U_inf,rho,bspv,frequency,beta_sq,B,kine_visc,
                          Theta,tau_w,dp_dx,delta,delta_star,Ue,p_ref,settings) for i in range(num_mic)] 
            
            mi = 0
            for instance in concurrent.futures.as_completed(results):
                res = instance.result()      
                broadband_noise_results.p_pref_broadband[:,mi,:,:]                               =  res.p_pref_broadband                         
                broadband_noise_results.p_pref_broadband_dBA[:,mi,:,:]                           =  res.p_pref_broadband_dBA                     
                broadband_noise_results.SPL_prop_broadband_spectrum[:,mi,:,:]                    =  res.SPL_prop_broadband_spectrum              
                broadband_noise_results.SPL_prop_broadband_spectrum_dBA[:,mi,:,:]                =  res.SPL_prop_broadband_spectrum_dBA          
                broadband_noise_results.SPL_prop_broadband_1_3_spectrum[:,mi,:,:]                =  res.SPL_prop_broadband_1_3_spectrum          
                broadband_noise_results.SPL_prop_broadband_1_3_spectrum_dBA[:,mi,:,:]            =  res.SPL_prop_broadband_1_3_spectrum_dBA      
                broadband_noise_results.p_pref_azimuthal_broadband[:,mi,:,:,:]                   =  res.p_pref_azimuthal_broadband               
                broadband_noise_results.p_pref_azimuthal_broadband_dBA[:,mi,:,:,:]               =  res.p_pref_azimuthal_broadband_dBA           
                broadband_noise_results.SPL_prop_azimuthal_broadband_spectrum[:,mi,:,:,:]        =  res.SPL_prop_azimuthal_broadband_spectrum    
                broadband_noise_results.SPL_prop_azimuthal_broadband_spectrum_dBA[:,mi,:,:,:]    =  res.SPL_prop_azimuthal_broadband_spectrum_dBA             
...
def compute_mic_broadband(i,r,num_cpt,num_rot,num_azi,num_cf,num_sec,blade_chords,delta_r,M,c_0,Omega,U_inf,rho,bspv,frequency,beta_sq,B,kine_visc,
                          Theta_mic,tau_w_mic,dp_dx_mic,delta_mic,delta_star_mic,Ue_mic,p_ref,settings):
...
def compute_rotor_section_boundary_layers(i,rotor,num_azi,Re_blade,alpha_blade,num_sec,U_blade,rho_blade,blade_chords,num_mic,num_rot,num_cf):
    ...
    boundary_layer.lower_surface_theta      = np.tile(lower_surface_delta[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # lower surface boundary layer thickness
    boundary_layer.upper_surface_theta      = np.tile(upper_surface_delta[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # upper surface boundary layer thickness
    boundary_layer.lower_surface_delta      = np.tile(lower_surface_delta_star[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf)) # lower surface displacement thickness
    boundary_layer.upper_surface_delta      = np.tile(upper_surface_delta_star[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf)) # upper surface displacement thickness
    boundary_layer.lower_surface_delta_star = np.tile(lower_surface_dp_dx[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # lower surface pressure differential
    boundary_layer.upper_surface_delta_star = np.tile(upper_surface_dp_dx[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # upper surface pressure differential
    boundary_layer.lower_surface_cf         = np.tile(lower_surface_Ue[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))         # lower surface boundary layer edge velocity
    boundary_layer.upper_surface_cf         = np.tile(upper_surface_Ue[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))         # upper surface boundary layer edge velocity
    boundary_layer.lower_surface_dp_dx      = np.tile((lower_surface_cf*(0.5*rho_blade[i]*(U_blade[i]**2)))[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))       # lower surface wall shear stress
    boundary_layer.upper_surface_dp_dx      = np.tile((upper_surface_cf*(0.5*rho_blade[i]*(U_blade[i]**2)))[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # upper surface wall shear stress
    boundary_layer.lower_surface_Ue         = np.tile(lower_surface_theta[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # lower surface momentum thickness
    boundary_layer.upper_surface_Ue         = np.tile(upper_surface_theta[None,None,:,:,None],(num_mic,num_rot,1,1,num_cf))      # upper surface momentum thickness
