...
def compute_blade_section_source_coordinates(AoA,acoustic_outputs,rotors,mls,settings):  
    ...
    if settings.parallel_computing:
        ...
        blade_section_position_vectors.sin_alpha_eff                   = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2)) 
        blade_section_position_vectors.cos_alpha_eff                   = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2))
        blade_section_position_vectors.M_hub_X                         = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2)) 
        blade_section_position_vectors.M_hub_Y                         = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2)) 
        blade_section_position_vectors.M_hub_Z                         = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2))  
        blade_section_position_vectors.cos_t_v                         = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2))  
        blade_section_position_vectors.sin_t_v                         = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2)) 
        blade_section_position_vectors.cos_t_v_t_r                     = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2))  
        blade_section_position_vectors.sin_t_v_t_r                     = np.zeros((num_cpt,num_mic,num_rot,num_sec,num_azi,num_cf,2))  
        
        
    
        pool   = multiprocessing.Pool(processes= num_processors)  
        data_list = list(np.arange(num_mic))        
        prod_x = partial(blade_section_cooridinates,
                         rot_origins = rot_origins,
                         body2thrust = body2thrust,
                         AoA  = AoA,
                         num_cf = num_cf,
                         r = r,
                         num_sec = num_sec,
                         ...
                         mls = mls,
                         phi_2d0 = phi_2d0,
                         alpha_eff0 = alpha_eff0 ,
                         num_azi = num_azi,
                         num_cpt = num_cpt)   
        single_mic_blade_section_vectors  = pool.map(prod_x, data_list)   
        for mi in range(num_mic):
            blade_section_position_vectors.blade_section_coordinate_sys[:,mi,:,:,:,:,:,:]    =  single_mic_blade_section_vectors[mi].blade_section_coordinate_sys 
            blade_section_position_vectors.vehicle_coordinate_sys[:,mi,:,:,:,:,:,:]          =  single_mic_blade_section_vectors[mi].vehicle_coordinate_sys       
            blade_section_position_vectors.cos_phi[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].cos_phi     
            blade_section_position_vectors.sin_alpha_eff[:,mi,:,:,:,:,:]                     =  single_mic_blade_section_vectors[mi].sin_alpha_eff 
            blade_section_position_vectors.cos_alpha_eff[:,mi,:,:,:,:,:]                     =  single_mic_blade_section_vectors[mi].cos_alpha_eff 
            blade_section_position_vectors.M_hub_X[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].M_hub_X    
            blade_section_position_vectors.M_hub_Y[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].M_hub_Y    
            blade_section_position_vectors.M_hub_Z[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].M_hub_Z    
            blade_section_position_vectors.cos_t_v[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].cos_t_v    
            blade_section_position_vectors.sin_t_v[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors[mi].sin_t_v    
            blade_section_position_vectors.cos_t_v_t_r[:,mi,:,:,:,:,:]                       =  single_mic_blade_section_vectors[mi].cos_t_v_t_r 
            blade_section_position_vectors.sin_t_v_t_r[:,mi,:,:,:,:,:]                       =  single_mic_blade_section_vectors[mi].sin_t_v_t_r 
     
                
        #with concurrent.futures.ProcessPoolExecutor(max_workers = num_processors) as executor:  
            #results = [executor.submit(blade_section_cooridinates,i,rot_origins,body2thrust,AoA,num_cf,r,num_sec,num_rot,mls,
                                       #phi_2d0, alpha_eff0,num_azi,num_cpt) for i in range(num_mic)] 
            #mi = 0
            #for instance in concurrent.futures.as_completed(results):
                ...
                #blade_section_position_vectors.cos_phi[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.cos_phi     
                #blade_section_position_vectors.sin_alpha_eff[:,mi,:,:,:,:,:]                     =  single_mic_blade_section_vectors.sin_alpha_eff 
                #blade_section_position_vectors.cos_alpha_eff[:,mi,:,:,:,:,:]                     =  single_mic_blade_section_vectors.cos_alpha_eff 
                #blade_section_position_vectors.M_hub_X[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.M_hub_X    
                #blade_section_position_vectors.M_hub_Y[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.M_hub_Y    
                #blade_section_position_vectors.M_hub_Z[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.M_hub_Z    
                #blade_section_position_vectors.cos_t_v[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.cos_t_v    
                #blade_section_position_vectors.sin_t_v[:,mi,:,:,:,:,:]                           =  single_mic_blade_section_vectors.sin_t_v    
...
def blade_section_cooridinates(i,rot_origins,body2thrust,AoA,num_cf,r,num_sec,num_rot,mls, phi_2d0, alpha_eff0,num_azi,num_cpt): 

    # Update dimensions for computation   
    r                    = np.tile(r[None,None,:,None,None],(num_cpt,num_rot,1,num_azi,num_cf))
    sin_phi              = np.tile(np.sin(phi_2d0)[:,None,:,:,None,None],(1,num_rot,1,1,num_cf,1))
    cos_phi              = np.tile(np.cos(phi_2d0)[:,None,:,:,None,None],(1,num_rot,1,1,num_cf,1))
    sin_alpha_eff        = np.tile(np.sin(alpha_eff0)[:,None,:,:,None,None],(1,num_rot,1,1,num_cf,1))
    cos_alpha_eff        = np.tile(np.cos(alpha_eff0)[:,None,:,:,None,None],(1,num_rot,1,1,num_cf,1))
    cos_t_v              = np.tile(np.cos(-AoA)[:,None,None,None,None,:],(1,num_rot,num_sec,num_azi,num_cf,1))
    sin_t_v              = np.tile(np.sin(-AoA)[:,None,None,None,None,:],(1,num_rot,num_sec,num_azi,num_cf,1))  
    cos_t_v_t_r          = np.tile(np.array([body2thrust[0,0]])[:,None,None,None,None,None],(num_cpt,num_rot,num_sec,num_azi,num_cf,1))  
    sin_t_v_t_r          = np.tile(np.array([body2thrust[0,2]])[:,None,None,None,None,None],(num_cpt,num_rot,num_sec,num_azi,num_cf,1))  
    M_hub                = np.tile(rot_origins[None,:,None,None,None,:,None],(num_cpt,1,num_sec,num_azi,num_cf,1,1))
    POS_2                = np.tile(mls[:,i,None,None,None,None,:,None],(1,num_rot,num_sec,num_azi,num_cf,1,1))

    # ------------------------------------------------------------
    # ****** COORDINATE TRANSFOMRATIONS ******  
    M_t      = np.zeros((num_cpt,num_rot,num_sec,num_azi,num_cf,3,3))
    M_phi    = np.zeros((num_cpt,num_rot,num_sec,num_azi,num_cf,3,3))
    M_theta  = np.zeros((num_cpt,num_rot,num_sec,num_azi,num_cf,3,3))
    M_tv     = np.zeros((num_cpt,num_rot,num_sec,num_azi,num_cf,3,3))
    ...
    single_mic_blade_section_position_vectors = Data()
    single_mic_blade_section_position_vectors.blade_section_coordinate_sys    = POS 
    single_mic_blade_section_position_vectors.vehicle_coordinate_sys          = POS_2
    single_mic_blade_section_position_vectors.cos_phi                         = np.repeat(cos_phi,2,axis = 5)  
    single_mic_blade_section_position_vectors.sin_alpha_eff                   = np.repeat(sin_alpha_eff,2,axis = 5)     
    single_mic_blade_section_position_vectors.cos_alpha_eff                   = np.repeat(cos_alpha_eff,2,axis = 5) 
    single_mic_blade_section_position_vectors.M_hub_X                         = np.repeat(M_hub[:,:,:,:,:,0,:],2,axis = 5)
    single_mic_blade_section_position_vectors.M_hub_Y                         = np.repeat(M_hub[:,:,:,:,:,2,:],2,axis = 5)
    single_mic_blade_section_position_vectors.M_hub_Z                         = np.repeat(M_hub[:,:,:,:,:,2,:],2,axis = 5)
    single_mic_blade_section_position_vectors.cos_t_v                         = np.repeat(cos_t_v,2,axis = 5)
    single_mic_blade_section_position_vectors.sin_t_v                         = np.repeat(sin_t_v,2,axis = 5)
    single_mic_blade_section_position_vectors.cos_t_v_t_r                     = np.repeat(cos_t_v_t_r,2,axis = 5)
    single_mic_blade_section_position_vectors.sin_t_v_t_r                     = np.repeat(sin_t_v_t_r,2,axis = 5)

    return single_mic_blade_section_position_vectors